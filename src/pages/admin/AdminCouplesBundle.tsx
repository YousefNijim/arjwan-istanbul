...
const newPerfume = (): PerfumeCard => ({
  id: `perf-${Date.now()}`,
  image: '', name: '', description: '', size: '100ml', badge: '', badgeColor: 'gold',
});

// ── Main page ─────────────────────────────────────────────────────────────────
const AdminCouplesBundle = () => {
  const [s, setS] = useState<BundleSettings>(DEFAULTS);
  ...
  useEffect(() => {
    adminApi.getSettings().then(data => {
      if (data?.couplesBundle) {
        setS(prev => ({ ...prev, ...data.couplesBundle }));
      }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await adminApi.updateSettings({ ...current, couplesBundle: s });
      setSaved(true);
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="font-display text-2xl text-foreground tracking-wider">Couples Bundle</h1>
        <p className="text-muted-foreground/50 text-xs mt-1">Controls the /bundles/couples-eid page</p>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-6 max-w-2xl">
        ...
        <section className="bg-card border border-border rounded-sm p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Hero Title" hint="Large heading in the hero">
        ...
        <section className="bg-card border border-border rounded-sm p-6 space-y-4">
          ...
          <Field label="Tagline" hint="Shown below the title">
        ...
        <section className="bg-card border border-border rounded-sm p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="End Date" hint="Countdown target date">
              <Input type="date" value={s.countdownDate} onChange={e => set('countdownDate', e.target.value)} />
            </Field>
            <Field label="Countdown Label" hint="Text above the timer">
        ...
        <section className="bg-card border border-border rounded-sm p-6 space-y-4">
          ...
          <p className="text-xs text-muted-foreground/50">Each card shown inside the bundle page</p>
...
export default AdminCouplesBundle;
