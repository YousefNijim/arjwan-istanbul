...
const Footer = () => {
  ...
  const footerText = lang === 'ar' ? siteSettings?.footerTextAr : lang === 'tr' ? siteSettings?.footerTextTr : siteSettings?.footerTextEn;
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
          ...
          <div className="text-center md:text-start">
            <Link to="/" className="inline-block">
              <span className="text-primary font-display text-xl tracking-[0.3em]">{siteSettings?.logoText || 'ARJWAN'}</span>
              <p className="text-foreground/40 text-xs tracking-[0.5em] uppercase">{siteSettings?.logoSubtext || 'Istanbul'}</p>
            </Link>
            {footerText && <p className="text-muted-foreground text-sm mt-3">{footerText}</p>}
          </div>
          ...
          <div className="flex flex-col items-center md:items-end gap-3">
            ...
            <div className="flex gap-4">
              {siteSettings?.instagramHandle && (
                <a href={`https://instagram.com/${siteSettings.instagramHandle}`} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-primary transition-colors">
                  <InstagramIcon size={20} />
                </a>
              )}
              {siteSettings?.tiktokHandle && (
                <a href={`https://www.tiktok.com/@${siteSettings.tiktokHandle}`} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-primary transition-colors">
              ...
              {(!siteSettings?.instagramHandle && !siteSettings?.tiktokHandle) && (
                <>
